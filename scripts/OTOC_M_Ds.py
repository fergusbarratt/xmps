...
from tdvp.ex import sympmat, williamson, tr_symp
...
T = linspace(0, 10, 400)
N = 8
fig, ax = plt.subplots(2, len(list(range(2, N+1))), sharex=True, sharey='row', figsize=(7.5, 7.5))
for q, D in enumerate(range(2, N+1)):
    i = j = L//2
    ...
    def bosonic_renyi(M):
        nu = williamson(tr_symp(M.T@M, 0))[0]
    ...
    for k in range(bosons_on_site[i]):
        ...
        botocs.append(botoc)
    botocs = array(botocs).T
    ax[0][q].plot(T, botocs, linewidth=1.3, label='bosons')
    ax[0][q].plot(T, mean(botocs.T, axis=0), linewidth=2.0, c='blue')
    #ax[i].legend(loc=1, bbox_to_anchor=(1.2, 1))
    ax[0][q].set_title('D={}, site {}'.format(D, i+1))
    ax[0][q].plot(T, OTOC, linewidth=1.5, c='black', label='spins')
    ax[1][q].plot(T, As.von_neumann())
    ax[1][q].plot(T, As_.von_neumann())
