...
from numpy import flip as fl, sqrt, exp
fig, ax = plt.subplots(1, 1)
x = array(range(len(numdiff(log(C[1:])))))


def f(x, a, b, c): return 1/(a*exp(b*x)-c)

params, _ = curve_fit(f, x[1:], numdiff(log(C[1:]))[1:])

ax.plot(x, numdiff(log(C[1:])), label='$\partial_t ln(C(t)$')
ax.plot(f(x, *params))
plt.show()

def g(x, a, b, c): return 1/(c*exp(b*x)+a)
#def g(x, a, b): return a*x**b
x_ = array(range(len(2*max_l(rm(D(S))))))
plt.plot(x_, 2*max_l(rm(D(S))))
params_, _ = curve_fit(g, x_, 2*max_l(rm(D(S))))
print(params_, params)
plt.plot(x_, g(x_, *params_))
plt.show()
#
#x = range(len(numdiff(log(C[1:]))))
#ax.plot(ks(rm(D(S)))/factor, label='$S_{KS}(D(t))/(D(t)^2)$')
